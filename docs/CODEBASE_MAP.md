...
| **Robot** | Raft sync tick | **Every 500 ms** | Iterates all reachable peers from the routing table (including multi-hop). Serializes Raft RPCs, sends via `MeshRouter.SendMessage` through `PeerService.RouteMessage` with per-hop network constraints. Quorum math uses the fixed startup `totalNodes` value, not the current reachable peer count. |
...
| [008](adr/008-robot-functionality.md) | Robot Functionality | Movement decisions based on local knowledge state, verified casualties, and Raft commit integration for committed casualties. |
