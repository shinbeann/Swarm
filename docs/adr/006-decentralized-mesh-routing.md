...
6. **Fixed quorum from startup membership**: Raft quorum is computed from a fixed swarm size established at startup. Each robot receives `totalNodes` once, keeps it unchanged, and uses that value for vote majorities and log-commit majorities. Routing knowledge still converges gradually through gossip, but it only affects message delivery, not quorum thresholds.
...
- Quorum size is fixed and identical across robots, so leadership and commit decisions remain anchored to startup membership even while routing paths are still converging.
