...
#### Raft Quorum Model
This Raft implementation uses a fixed membership size established at startup. Each robot receives the swarm size as configuration (`totalNodes`) and keeps it unchanged for the lifetime of the process. Routing tables still determine which peers are currently reachable for message delivery, but vote majorities and log-commit majorities are computed against the fixed startup size rather than the current reachable set.

The implication is that all robots agree on the same quorum threshold, while network partitions can still stall elections and commits if fewer than a majority of the startup members remain reachable.
